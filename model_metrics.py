...
for i in range(len(llind)):
	...
	if  tmin < tmax:
		...
		mubar_i = np.interp(dtimes, mtimes, mubar[:,llind[i]])
		mvbar_i = np.interp(dtimes, mtimes, mvbar[:,llind[i]])	
		msu = model_skill(mubar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())], ubar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])
		msv = model_skill(mvbar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())], vbar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])
		biasu = np.mean(mubar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())]-ubar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])
		biasv = np.mean(mvbar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())]-vbar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])
		r2u = np.corrcoef(mubar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())], ubar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])[0,1]**2
		r2v = np.corrcoef(mvbar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())], vbar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])[0,1]**2
		rmsu = np.mean(np.sqrt((mubar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())] - ubar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])**2))
		rmsv = np.mean(np.sqrt((mvbar_i[(time >= tmin.timestamp()) & (time <= tmax.timestamp())] - vbar[(time >= tmin.timestamp()) & (time <= tmax.timestamp())])**2))
		...
		for k in range(len(dep)):
			...
			vtime = time[~np.isnan(dumv)]
			msu = model_skill(munan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())], unan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())])
			msv = model_skill(mvnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())], vnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())])
			biasu = np.mean(munan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())]-unan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())])
			biasv = np.mean(mvnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())]-vnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())])
			r2u = np.corrcoef(munan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())], unan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())])[0,1]**2
			r2v = np.corrcoef(mvnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())], vnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())])[0,1]**2
			rmsu = np.mean(np.sqrt((munan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())] - unan[(utime >= tmin.timestamp()) & (utime <= tmax.timestamp())])**2))
			rmsv = np.mean(np.sqrt((mvnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())] - vnan[(vtime >= tmin.timestamp()) & (vtime <= tmax.timestamp())])**2))
